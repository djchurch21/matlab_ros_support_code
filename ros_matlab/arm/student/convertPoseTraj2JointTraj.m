function [mat_joint_traj, robot_joint_names] = convertPoseTraj2JointTraj(mat_traj, optns)
    ...
    % Extract robot
    r = optns('rHandle');  % Corrected access to the robot handle in optns
    ...
    for i = 1:num_traj_points
        [des_q, ~] = r.ik('tool0', mat_traj(:,:,i), r.ik_weights, mat_cur_q); 
        ...
        % Print solution for debugging purposes
        fprintf('IKs: '); fprintf('%.2f,', des_q); fprintf('\n');
        
        % Set the joint angles of the trajectory
        mat_joint_traj(i,:) = des_q(1,:);

        % Update joint angles for next round
